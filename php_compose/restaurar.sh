...
if [[ -n "$BACKUP_FILE" ]]; then
  ...
  docker run --rm \
    ...
    sh -c "\
      ...
      rm -rf /var/lib/mysql/*; \
      echo '    • Ejecutando tar xzvf /respaldo/$BNAME -C /var/lib/mysql --strip-components=3 ...'; \
      tar xzvf /respaldo/$BNAME -C /var/lib/mysql --strip-components=3; \
...
# Usamos override para evitar que MySQL intente 'initialize' sobre un directorio lleno

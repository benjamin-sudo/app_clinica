<style>
    .table  {
        border-collapse:collapse;
    }
</style>
<table style="width:100%">
    <tr>
        <td style="width:50%">
            <img class="img-thumbnail" title="" alt="" style="width:100px;height:90px" src="data:image/jpeg;base64," /> 
            <br>
            <b>SERVICIO DE SALUD ARAUCANIA NORTE</b><br>
            <b><?php echo $empresa=='100'?'HOSPITAL DR. MAURICIO HEYERMANN':'HOSPITAL SAN JOSE DE VICTORIA';?></b>
            <br>
            <b>CENTRO DE COSTO ANATOM&Iacute;A PATOL&Oacute;GICA</b><br> 
            <b>BLO/VRR</b>
        </td>
        <td style="width:50%">&nbsp;</td>
    </tr>
</table>

<table style="width: 100%">
    <tr>
        <td style="width: 50%">&nbsp;</td>
        <td style="width: 50%">
            <b>MEMO.:</b>&nbsp; N&deg; 01<br>
            <b>MAT.:</b>&nbsp;Rechazo ilegibilidad de biopsias<br>  
            <?php echo $empresa=='100'?'<b>ANGOL</b>':'<b>VICTORIA</b>'?>,
            <br>
            <b>FECHA DE RECHAZO.:</b> <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['FEC_REVISION'];?>
        </td>
    </tr>
</table>

<table style="width: 100%">
    <tr>
        <td style="width: 50%">
            <table style="width: 100%">
                <tr>
                    <td style="width: 20%" valign="top"><b>DE</b></td>
                    <td style="width: 5%"  valign="top">:</td>
                    <td style="width: 65%">
                        DR. BOLIVAR LEE OLMOS<br>
                        JEFE ANATOMIA PATOLOGICA<br>
                        <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_HOSPITAL_ETI'];?>
                    </td>
                </tr>
                <tr>
                    <td valign="top"><b>A</b></td>
                    <td valign="top">:</td>
                    <td>
                        <?php  if($DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['PA_ID_PROCARCH'] == '31'){  ?>
                            <?php if($empresa==='100'){ ?>
                               EU. XIMENA SANDOVAL<br>
                               COORDINADORA PABELL&Oacute;N<br> 
                               HOSPITAL ANGOL
                            <?php } else { ?>
                               EU. EUGENIA ROSAS<br>
                               COORDINADORA PABELL&Oacute;N<br> 
                               HOSPITAL VICTORIA
                            <?php } ?>
                        <?php   }   else    {   ?>
                            <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['PROFESIONAL']." ".$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['RUT_PROFESIOAL'];?> <br>   
                            <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_PROFESIONAL'];?><br>    
                            <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_HOSPITAL_ETI'];?>
                        <?php   }   ?>
                    </td>
                </tr>    
            </table>
        </td>
        <td style="width: 50%">&nbsp;</td>
    </tr>
</table>
<br>
<table style="width: 100%">
    <tr>
        <td style="width: 100%">
            Junto con saludarle, inform&oacute; a Ud., 
            que se rechazan las siguientes biopsias debido a la ilegibilidad de los registros. 
            Solicito hacer hincapi&eacute; en la revisi&oacute;n y correcto registro de los datos de nuestros pacientes.
            <br>
            <br>
            <b>TIPO DE BIOPSIA</b> 
            <br>
            <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TIPO_DE_BIOPSIA'];?>
        </td>
    </tr>
</table>
<br>
<table style="width: 100%;border-collapse: collapse;" class="table">
    <tr>
        <td style="width: 50%">
            <b>NOMBRES Y APELLIDOS</b> 
        </td>
        <td style="width: 50%">
            <b>MOTIVO DE RECHAZO</b>
        </td>
    </tr>
    <tr>
        <td>
            <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NOMBRE_COMPLETO'];?> | <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['RUTPACIENTE'];?>
        </td>
        <td>
            <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_OBS_RECHAZA'];?>
        </td>
    </tr>
</table>

<br>
<table style="width: 100%;border-collapse: collapse;" class="table">
    <tr>
        <td colspan="2" style="text-align:left;"><b>LISTA DE MUESTRAS RECHAZADAS</b></td>
    </tr>
    <tr>
        <?php if (count($DATA['P_INFO_LOG_ADVERSOS'])>0){ ?>
            <?php foreach($DATA['P_INFO_LOG_ADVERSOS'] as $i => $row){ ?>
                <td style="width: 50%">
                    <?php echo ($i+1)." -  | N&deg; ".$row['ID_NMUESTRA']." | ".$row['DESCRIPCION'];?> 
                </td>
                <td style="width: 50%">
                   <?php echo $row['TXT_EVENTO_OBSERVACION'];?> 
                </td>
            <?php }  
        } ?>
    </tr>
</table>
<br>
<table style="width:100%">
    <tr>
        <td style="width: 50%">
           Sin otro particular, saluda atentamente a Ud. 
        </td>
        <td style="width: 50%">&nbsp;</td>
    </tr>
    <tr>
        <td>&nbsp;</td>
        <td>
            DR. BOLIVAR LEE OLMOS<br>
            JEFE ANATOMIA PATOLOGICA<br>
            <b>HOSPITAL</b> 
            <?php echo $empresa=='100'?'<b>MAURICIO HEYERMANN ANGOL</b>':'<b>SAN JOSE DE VICTORIA</b>'?>
        </td>
    </tr>
    <tr>
        <td>
            DISTRIBUCI&Oacute;N<br>
            <div style="margin-left: 10px">
                - La Indicada<br>
                - Archivo Anatom&iacute;a Patol&oacute;gica
            </div>
        </td>
        <td>&nbsp;</td>
    </tr>   
</table>    

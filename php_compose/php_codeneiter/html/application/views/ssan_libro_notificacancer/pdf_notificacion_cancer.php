<?php
$arr_info_muestras                  =   [];
$arr_info_cito                      =   [];
#todas las muestras de anatomia
if(count($DATA['P_ANATOMIA_PATOLOGICA_MUESTRAS'])>0){
    $IND_USOCASSETTE    =   $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['IND_USOCASSETTE'];
    if($IND_USOCASSETTE == '1'){
        //echo "Si Casete";
        foreach($DATA["P_ANATOMIA_PATOLOGICA_MUESTRAS"] as $i => $row){ 
            //$ARR_CASETE_ORD[$row['ID_CASETE']][] = $row; 
            array_push($arr_info_muestras,'C:'.$row['NUM_CASSETTE'].'.- '.$row["TXT_MUESTRA"]); 
        }
    } else {
        foreach($DATA['P_ANATOMIA_PATOLOGICA_MUESTRAS'] as $i => $row){
            $aux_muestras           =   ($i+1);
            array_push($arr_info_muestras,$aux_muestras.".- ".$row["TXT_MUESTRA"]);
        }
    }
}
#muestrs de citologia
if(count($DATA['P_AP_MUESTRAS_CITOLOGIA'])>0){
    $aux_cito                       =   1;
    foreach ($DATA['P_AP_MUESTRAS_CITOLOGIA'] as $i => $row){
        array_push($arr_info_cito,$aux_cito.".- ".$row["TXT_MUESTRA"]);
    }
}
?>

<table style="width:100%">
    <tr>
        <td style="width:10%" valign="top">
            <img class="img-thumbnail" title="" alt="" style="width:100px;height:90px" src="data:image/jpeg;base64," /> 
        </td>
        <td style="width:35%" valign="top">
            <b>SERVICIO DE SALUD ARAUCAN&Iacute;A NORTE</b><br>
            <b><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_HOSPITAL_ETI'];?></b><br>
            <b>UNIDAD ANATOMIA ANATOM&Iacute;A PATOL&Oacute;GICA</b><br> 
            <b>BLO/VRR</b>
        </td>
        <td style="width:50%;text-align:left" valign="top">
            &nbsp;<h2><b><title>NOTIFICACI&Oacute;N DE CANCER</title></b></h2></td>
        <td style="width:40%;text-align:start" valign="top" s >
            <!-- CAMBIAR FECHA POR FECHA DE 
            <p> </p>
            <?php  
                $fechaHD    =   explode("-",$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['FECHA_RECEPCION']); 
                $dias       =   array("DOMINGO","LUNES","MARTES","MIERCOLES","JUEVES","VIERNES","SABADO");
                $meses      =   array("ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","AGOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE");
                echo $this->session->userdata["nomestab"].",".$dias[date("w",mktime(0,0,0,$fechaHD[1],$fechaHD[0],$fechaHD[2]))]." ".$fechaHD[0]." DE ".$meses[$fechaHD[1]-1]. " DEL ".$fechaHD[2];
            ?>
            -->
        </td>
    </tr>
</table>

<br>

<style>
    .border                 {
        border-collapse     :   collapse;
        border              :   1px solid black;
        font-size           :   12px;
      
    }
    .space                  {
        border-spacing      :   0px;
        font-size           :   12px;
    }
    .td {
        border              :   1px solid black;
        height              :   40px;
    }
</style>

<?php
    $date_trasporte         =   new DateTime(date('d-m-Y h:i:s',strtotime($DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['FECHA_TRASLADO']." ".$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['HORA_TRASLADO'].":00")));
    $date_recepcion         =   new DateTime(date('d-m-Y h:i:s',strtotime($DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['FECHA_RECEPCION']." ".$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['HORA_RECEPCION'].":00")));
    $diff                   =   $date_recepcion->diff($date_trasporte); 
    $num_dias               =   $diff->days;
    $num_hora               =   $diff->h;
    $num_minutos            =   $diff->i;
?>

<table style="width:100%" class="space border">
    <thead>
        <tr>
            <td style="width:12%;background-color:#9b9b9b;height:40px;">&nbsp;N&deg;</td>
            <td style="width:4%;background-color:#9b9b9b;">&nbsp;N&deg; MEMO</td>
            <td style="width:15%;background-color:#9b9b9b;">&nbsp;NOMBRE PACIENTE</td>
            <td style="width:20%;background-color:#9b9b9b;">&nbsp;TIPO DE MUESTRA</td>
            <td style="width:10%;background-color:#9b9b9b;">&nbsp;FECHA/HORA EMISI&Oacute;N INFORME</td>
            <td style="width:10%;background-color:#9b9b9b;">&nbsp;FECHA/HORA NOTIFICACI&Oacute;N</td>
            <td style="width:19%;background-color:#9b9b9b;">&nbsp;NOMBRE DE FUNCIONARIO QUE NOTIFICA</td>
            <td style="width:19%;background-color:#9b9b9b;">&nbsp;NOMBRE DE FUNCIONARIO QUE RECIBE NOTIFICACI&Oacute;N</td>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td style="" class="td" valign="top">
                <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_INTERNO_AP']=='0'?'':'N&deg; BIOPSIA: '.$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_INTERNO_AP'];?> 
                <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['IND_TIPO_BIOPSIA']== 4?'<br>':'';?> 
                <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_CO_CITOLOGIA']==''?'':'N&deg; CITOL&Oacute;GICO: '.$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_CO_CITOLOGIA'];?> 
                <?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_CO_PAP']==''?'':'N&deg; PAP: '.$DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_CO_PAP'];?> 
            </td>
            <td style="" class="td" style="text-align: center" valign="top"><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NUM_NOF_CANCER'];?></td>
            <td style="" class="td" valign="top"><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['NOMBRE_COMPLETO'];?></td>
            <td style="" class="td" valign="top">
                <?php 
                    #echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['DES_SITIOEXT'];
                ?>
                <?php echo count($arr_info_muestras)>0?join("<br>",$arr_info_muestras):''?>
                <?php echo count($arr_info_muestras)>0?'<br>':'';?>
                <?php echo count($arr_info_cito)>0?join("<br>",$arr_info_cito):''?>
            </td>
            <td style="text-align: center" class="td" valign="top"><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['DATE_FECHA_DIAGNOSTICO'];?></td>
            <td style="text-align: center" class="td" valign="top"><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['DATE_NOTIFICACANCER'];?></td>
            <td style="" class="td" valign="top"><b><font size="2">FIRMA DIGITAL DE:</b><br><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_NOTIFICA_CANCER'];?></td>
            <td style="" class="td" valign="top"><b><font size="2">FIRMA DIGITAL DE:</b><br><?php echo $DATA['P_ANATOMIA_PATOLOGICA_MAIN'][0]['TXT_RECIBE_NOTIFICA_CANCER'];?></td>
        </tr>
    </tbody>
</table>

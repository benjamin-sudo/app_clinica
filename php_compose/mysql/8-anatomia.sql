CREATE TABLE ADMIN.PB_SOLICITUD_HISTO (
  ...
  DES_BIPSIA VARCHAR(1000),
  DES_CITOLOGIA VARCHAR(1000),
  DES_OBSERVACIONES VARCHAR(1000),
  IND_ESTADO INT,
  ...
  TXT_DIADNOSTICO_AP VARCHAR(4000),
  ...
  TXT_DESC_MACROSCOPICA VARCHAR(1000),
  ...
  TXT_CITOLOGICO VARCHAR(4000),
  NUM_CO_CITOLOGIA INT,
  NUM_CO_PAP INT,
  TXT_DIAG_CITOLOGIA VARCHAR(4000),
...
BEGIN
    ...
    SELECT
        ...
        L.IND_TISEXO,
        L.COD_RUTPAC,
        TRUNCATE(MONTHS_BETWEEN(NOW(), L.FEC_NACIMI) / 12, 0) AS NUMEDAD,
        DATE_FORMAT(L.FEC_NACIMI, '%d-%m-%Y') AS NACIMIENTO,
        TRUNCATE(MONTHS_BETWEEN(NOW(), L.FEC_NACIMI) / 12, 0) AS EDAD,
